...
export default function AzurePage() {
  ...
  const handleScan = async () => {
    ...
    setCurrentJob(null);
    try {
      const { id } = await api.cloud.createScan("azure", creds);
      const job = await api.cloud.getScan(id);
      setCurrentJob(job);
      pollJob(id);
    } catch (e: unknown) {
      ...
      <main className="min-h-screen bg-background bg-dots">
        <div className="mx-auto max-w-4xl px-6 py-8 space-y-6">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground/60">
            ...
            <span className="text-muted-foreground">Azure Configuration Review</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground tracking-tight">Azure Configuration Review</h1>
              <p className="text-[12px] text-muted-foreground/60">Audit Storage, App Services, SQL, Key Vault, and NSG for misconfigurations.</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleScan} disabled={scanning}
              ...
              {scanning ? "Scanning…" : "Run Azure Review"}
            </button>
                ...
                <div className="p-5">
                  {currentJob.status === "running" || currentJob.status === "queued" ? (
                    <div className="flex items-center justify-center gap-3 py-12 text-muted-foreground">
                      <Loader2 className="h-5 w-5 animate-spin" /><span className="text-sm">Running cloudlist + prowler against Azure…</span>
                    </div>
